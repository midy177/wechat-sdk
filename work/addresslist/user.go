...
const (
	...
	// userIDGetByMobileURL 通过手机号获取用户ID
	userIDGetByMobileURL = "https://qyapi.weixin.qq.com/cgi-bin/user/getuserid"
	// userIDGetByEmailURL 通过邮箱获取用户ID
	userIDGetByEmailURL = "https://qyapi.weixin.qq.com/cgi-bin/user/get_userid_by_email"
	// userDeleteURL 删除成员
...
func (r *Client) GetUserIdByMobile(mobile string) (string, error) {
	var (
		accessToken string
		err         error
	)
	...
	err = util.DecodeWithError(response, result, "GetUserIdByMobile")
	return result.UserID, err
}

func (r *Client) GetUserIdByEmail(email string) (string, error) {
	var (
		accessToken string
		err         error
	)
